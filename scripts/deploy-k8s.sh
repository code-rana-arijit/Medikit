...
case "$ACTION" in
  full)
    ...
    for svc in discovery-server config-server api-gateway user-service product-service inventory-service cart-service order-service payment-service delivery-service notification-service prescription-service search-service health-service loyalty-service discount-service distributor-service; do
    ...
    kubectl rollout status deployment -n $NS user-service product-service inventory-service cart-service order-service payment-service delivery-service notification-service prescription-service search-service health-service loyalty-service discount-service distributor-service --timeout=300s
    ;;
  rollout-status)
